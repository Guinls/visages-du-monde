...
const CARD_WIDTH = 380; // doit matcher la largeur de la carte en desktop

/**
 * Section "Inspirations du moment" — slider horizontal éditorial.
 *
 * ⚠️ Pas un site marchand : on n'affiche AUCUN prix en gros.
 * Le tag (terracotta) sert à classer la proposition (univers, format) —
 * pas à promouvoir une remise. Les détails se découvrent en agence.
 */
export function Inspirations() {
  ...
  return (
    <section
      ...
      aria-label="Inspirations du moment"
    >
      <Container>
        <div className="flex flex-wrap items-end justify-between gap-6">
          <SectionReveal className="max-w-2xl">
            ...
            <p className="mt-5 text-body-lg text-slate">
              Sélection de nos coups de cœur saisonniers.
            </p>
          </SectionReveal>
          ...
          <div className="flex gap-2">
            <SliderButton
              direction="prev"
              ariaLabel="Voir les inspirations précédentes"
              onClick={() => scrollBy(-1)}
            />
            <SliderButton
              direction="next"
              ariaLabel="Voir les inspirations suivantes"
              onClick={() => scrollBy(1)}
            />
          </div>
        </div>
      ...
      <div className="mt-12">
        <ul
          ...
          aria-label="Liste des inspirations du moment"
        >
          {INSPIRATIONS.map((d) => (
            <InspirationCard key={d.id} item={d} />
          ))}
        </ul>
      </div>
    </section>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-3 line-clamp-3 text-[14px] leading-relaxed text-slate">
          {item.description}
        </p>
